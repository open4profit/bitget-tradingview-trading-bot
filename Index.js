...
app.post('/api/v1/placeorder', async (req, res) => {
    ...
    const tp = parseFloat(parseFloat(price) - (parseFloat(price) * (TAKE_PROFIT_PERCENT / leverage) / 100)).toFixed(2);
    const stoploss = parseFloat(parseFloat(price) + (parseFloat(price) * (STOP_LOSS_PERCENT / leverage) / 100)).toFixed(2);    
